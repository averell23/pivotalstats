class StatsAggregator
  ...
  def get_iterations(project)
    [].tap do |iterations|
      (0..@number_of_iterations).each { |off| iterations << project.iterations(scope: "done", offset: -off, limit: 1).first rescue nil }
    end
  end

  def add_iteration_stats(project_name, iteration_stat)
    iteration_stat.update!
    iterations[iteration_stat.finish] ||= {}
    iterations[iteration_stat.finish][project_name] = iteration_stat
  end
